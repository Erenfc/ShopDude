...
const RecipeIngredients = () => {
 ...
 useEffect(() => {
    const fetchProducts = async () => {
      const productsSnapshot = await fireDB.collection('products').get();
 ...
 const searchRecipes = () => {
    ...
    if (ingredients) {
      const matchedProducts = availableProducts.filter(product =>
        ingredients.some(ingredient => product.name.toLowerCase() === ingredient.toLowerCase())
      );
      setRecipes(matchedProducts);
      setErrorMessage('');
    } else {
      setRecipes([]);
        ...
        <div>
          {recipes.length > 0 ? (
            recipes.map((product, index) => (
              <ProductCard key={index} title={product.name} />
            ))
          ) : (
            <p></p>
          )}
